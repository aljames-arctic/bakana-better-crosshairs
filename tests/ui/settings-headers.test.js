...
test('injectSettingsHeaders inserts world, user, and client headers into BBC SettingsConfig DOM', () => {
    ...
    try {
        ...
        // World group (autorecExchangeMenu menu button)
        const fgWorld = new MockElement('div', { className: 'form-group' });
        const btnWorld = new MockElement('button', { dataset: { key: `${MODULE_ID}.autorecExchangeMenu` } });
        fgWorld.appendChild(btnWorld);
        root.appendChild(fgWorld);

        // User group (autorecMenu menu button)
        const fgUser = new MockElement('div', { className: 'form-group' });
        const btnUser = new MockElement('button', { dataset: { key: `${MODULE_ID}.autorecMenu` } });
        fgUser.appendChild(btnUser);
        root.appendChild(fgUser);

        // Client group (showOtherPlayersCrosshairs)
        const fgClient = new MockElement('div', { className: 'form-group' });
        const inputClient = new MockElement('input', { name: `${MODULE_ID}.showOtherPlayersCrosshairs` });
...
test('injectSettingsHeaders moves autorecMenu into the User Settings section', () => {
    const origCreateElement = globalThis.document.createElement;
    globalThis.document.createElement = (tagName) => new MockElement(tagName);
    ...
    try {
        ...
        // autorecMenu is registered first, followed by autorecExchangeMenu
        ...
        // [0]: World Header
        // [1]: fgExchangeMenu (World Menu)
        // [2]: fgBroadcasting (World Setting)
        // [3]: User Header
        // [4]: fgAutorecMenu (User Menu relocated here!)
        // [5]: Client Header
        // [6]: fgShowOther (Client Setting)
        // [7]: fgLog (Client Setting)
        assert.equal(root.children.length, 8);
        assert.equal(root.children[0].className, 'bbc-settings-section-header');
        assert.equal(root.children[0].dataset.scope, 'world');
        assert.equal(root.children[1], fgExchangeMenu);
        assert.equal(root.children[2], fgBroadcasting);

        assert.equal(root.children[3].className, 'bbc-settings-section-header');
        assert.equal(root.children[3].dataset.scope, 'user');
        assert.equal(root.children[4], fgAutorecMenu, 'autorecMenu should be relocated under User Settings');

        assert.equal(root.children[5].className, 'bbc-settings-section-header');
        assert.equal(root.children[5].dataset.scope, 'client');
        assert.equal(root.children[6], fgShowOther);
